...
hiden_lengthscale_choice_1 = [-4.5, -5.0,-6.0]
hiden_lengthscale_choice_2 = [-4.5, -5.0, -6.0]
...
# hiden_outputscale_choise = [0.1, 0.05, -0.05, -0.1,] # [-3.0, -2.0, -1.0] [-0.1, -0.01, 0.1]
# last_lengthscale_choice  = [-0.1, 0.1, -0.05] # last_lengthscale_choice  = [-0.1, 0.1, 0.01, -0.01]
# last_outputscale_choise  = [-0.1, -1.0, -1.25] # -1.5, -1.75, 0.01
# # likelihood_noise_range   = [0.1]
# likelihood_noise_range   = [0.01, 0.05]

# hiden_lengthscale_choice = [-4.5] # want to try -5.5, -4.5
# hiden_outputscale_choise = [0.01, -0.01,-1.0, -0.1]
# last_lengthscale_choice  = [3, 4, 5,]
# last_outputscale_choise  = [0.05, -0.1, -1.0]
# likelihood_noise_range   = [0.05, 1.5]

# ssh
# hiden_lengthscale_choice = [-4.5] # want to try -5.5, -4.5
# hiden_outputscale_choise = [0.01, -0.01,-1.0, -0.1]
# last_lengthscale_choice  = [-0.1, 0.1, 0.01,]
# last_outputscale_choise  = [0.05, -0.1, -1.0]
# likelihood_noise_range   = [0.05, 1.5]

# hiden_lengthscale_choice = [-4.25] # want to try -5.5
# hiden_outputscale_choise = [-2.0, -1.0, -0.1] # [-3.0, -2.0, -1.0]
...
for search_time in range(num_search):
    hidden_layer_lengthscale_1 = random.choice(hiden_lengthscale_choice_1)
    hidden_layer_lengthscale_2 = random.choice(hiden_lengthscale_choice_2)
    ...
    for time in range(repeat_time):
        ...
        for train_index, val_index in kf.split(train_x):
            ...
            hypers = {
                'hidden_layer.covar_module.base_kernel.raw_lengthscale': torch.tensor(
                    [[[hidden_layer_lengthscale_1, hidden_layer_lengthscale_2]]]).to(device),  # -3,
        ...
        print(grid_times,
              '. hidden_lengthscale: ', [hidden_layer_lengthscale_1, hidden_layer_lengthscale_2],
        ...
        if (total_train_rmse / cross_split) < best_train:
            ...
            best_hyperparams = {
                'hidden_layer_lengthscale': [hidden_layer_lengthscale_1, hidden_layer_lengthscale_2],
...
#                 for hidden_layer_lengthscale_1 in hiden_lengthscale_choice_1:
#                     for hidden_layer_lengthscale_2 in hiden_lengthscale_choice_2:
#                         for time in range(repeat_time):
#                             grid_times = grid_times + 1
#                             total_train_rmse = 0.0
#                             total_test_rmse = 0.0
...
#                                     'hidden_layer.covar_module.base_kernel.raw_lengthscale': torch.tensor(
#                                         [[[hidden_layer_lengthscale_1, hidden_layer_lengthscale_2]]]).to(device),  # -3,
#                                     'hidden_layer.covar_module.raw_outputscale': torch.tensor(
#                                         [hidden_layer_outputscale]).to(device),
#                                     'last_layer.covar_module.raw_outputscale': torch.tensor(
#                                         [last_layer_outputscale]).to(device),
#                                     # 0, 1, 0.5
#                                     'last_layer.covar_module.base_kernel.raw_lengthscale': torch.tensor(
#                                         [[[last_layer_lengthscale]]]).to(device),
#                                     'likelihood.raw_task_noises': torch.tensor([likelihood_noise_value]).to(device),
...
hypers = {
    'hidden_layer.covar_module.base_kernel.raw_lengthscale': torch.tensor([[best_hyperparams['hidden_layer_lengthscale']]]).to(device), #-3,
