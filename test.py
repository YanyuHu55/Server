...
for search_time in range(num_search):
    ...
    for time in range(0, 2):
        ...
        for train_index, val_index in kf.split(train_x):
            ...
            mll = DeepApproximateMLL(VariationalELBO(model.likelihood, model, num_data=train_y_cross.size(0)))
...
#                                 VariationalELBO(model.likelihood, model, num_data=train_y_cross.size(0)))
#                             num_epochs = 1 if smoke_test else 100
#                             # num_epochs = 1 if smoke_test else 2000 best
#                             epochs_iter = tqdm.tqdm(range(num_epochs), desc='Epoch')
#                             loss_set = np.array([])
