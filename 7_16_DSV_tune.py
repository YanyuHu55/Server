...
hiden_outputscale_choise = [-0.1]
last_lengthscale_choice  = [-0.01]
last_outputscale_choise  = [-1.0]
likelihood_noise_range   = [0.05]
num_inducing_choice = [i for i in range(70, 250, 3)]
                    ...
                    for num_inducing in num_inducing_choice:
                        ...
                        for time in range(repeat_time):
                            ...
                            hypers = {
                                ...
                                'hidden_layer.covar_module.raw_outputscale': torch.tensor(
                                    [hidden_layer_outputscale]).to(device),
                                'last_layer.covar_module.raw_outputscale': torch.tensor([last_layer_outputscale]).to(
                                    device),
                                # 0, 1, 0.5
                                'last_layer.covar_module.base_kernel.raw_lengthscale': torch.tensor(
                                    [[[last_layer_lengthscale]]]).to(device),
